# -*- coding: utf-8 -*-
"""
Created on Tue Dec  1 14:25:38 2020
@author: ukjo

부동산 뉴스 제목을 기준으로 한 (관련없는) 가짜 부동산 뉴스 필터링 알고리즘 

1. 데이터 : 아래 테이블에서 뉴스 제목(title) 가짜 여부(is_valid), 일자(yyyymmdd) 정도 조회
...
class DBInfo:
    def __init__(self):
        ...
        self.host = "richgoweb.cluster-czdphhgdzbon.ap-northeast-2.rds.amazonaws.com"
        self.user = "crawler2"
        self.pw = "cASEbnP#Cl_20201012"
        self.db = "richgo"
...
conn = pymysql.connect(host=db_info['host'], user=db_info['user'], passwd=db_info['passwd'], db=db_info['db'])
conn.set_charset('utf8mb4')
cur = conn.cursor()

cur.execute("select yyyymmdd,title,is_valid from richgo.realestate_news")
df = cur.fetchall()

df = pd.DataFrame(list(df), columns=['yyyymmdd',' title', 'is_valid'])
